...
function out = DFN_Ageing_JPS(input,final_time,init_cond,varargin)
...
epss_pos_temp = zeros(p.nnp,min(max_prealloc_size,round(time_max/p.dt))); % zeros(1,round(time_max/p.dt));  %
...
%     elseif not(max_iterations) && not(dt_or==p.dt) && kl2==0 
    ...
    if p.ageing   
        ...
        %Cathode Oxidation
        % BO = -0.001;
        epss_pos_temp(:,t+1) = epss_pos_temp(:,t) - p.dt*p.BO*j_ox(:,t+1);
    ...
    if mod(t,500)==0 && p.ageing==1
        ...
        p.epss_pos = epss_pos_temp(1+p.nn:end,t);
...
if p.ageing
    ...
    out.ageing.epss_pos = [epss_pos_temp(1:p.nn,2:n_t); NaN(p.ns,n_t-1); epss_pos_temp(p.nn+1:end,2:n_t)];
...
if p.ageing
    ...
    if p.ageingMech.LPL
            ...
            jlpl = (p.i0lpl/p.F).*(((FacLPL/5000).*exp(p.alphaa_lpl*p.F/(p.R*T)*eta2))-((CeRat/1000).*exp(-p.alphac_lpl*p.F/(p.R*T).*eta2)));
    ...
    if p.ageingMech.Oxidation
        ...
        j_ox = (p.i0C/p.F).*exp(p.alpha_ox*p.F/(p.R*T)*eta_ox);
    else 
...
if p.ageing
    ...
    dj_oxdphis = diag(p.i0C*p.alpha_ox/(p.R*T)*exp(p.alpha_ox*p.F/(p.R*T)*eta_ox))*deta_oxdphis;
...
p.U_neg = @(theta_n) 0.01+0.7222 + 0.1387*theta_n + 0.029*theta_n.^0.5 - 0.0172./theta_n + 0.0019./...
